%                                      DLEEER algorithm (first prposed
%                                    algoritm ) work after DL_LBR_algorithm_c

function [new_total_relability,new_total_number_of_replica,new_num,new_processor_available, result_relability_matrix,R,total_dynmic_power_consumed, result_replica_power,task_power_consumed,execution_time] =  DLEEER_algorithm_c()
format long;
tic;
...
%%% we call LBR algoritm in the first
 [total_number_of_replica,total_relability,R,processor_available,num,replica_matrix,~,est,drt,aft,processor_avail] = DL_LBR_algorithm_c();
...
for i=1:1:n
    for j=1:1:number_of_processors
        % calculate the energy consumption by the replica
                var1=(Pk_ind(j)+(Cef(j)*power(1,mk(j))));
                var2=(fk_max(j)/1);
                replica_energy_consumption =var1*W(i,j)*var2;
                
                beta_parameter =enhanced_app_relability(i)/( replica_energy_consumption * W(i,j) ); 
      if ( beta_parameter > flag_max_beta && new_processor_available(i,j) == 0 ) 
       flag_max_beta=beta_parameter;
