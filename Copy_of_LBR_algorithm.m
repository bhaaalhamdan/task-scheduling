          ...
          if (processor_available(i,j)==0 && flag_task==0)
            ...
            for v=1:1:fr_u_colum
             % task replica relability calculation from eq (6) in notebook
                ...
                frequency = fr_u(j,v);
                  if (replica_relability >= Rlb_req(i)&& min_frequency > frequency)
                   min_frequency= frequency; 
                   relability= replica_relability;
                   replica_matrix(i,j)=relability;
                   flag_relability=1;
                   processor_available(i,j)=1;
                   result_matrix(i,j)=fr_u(j,v);
                   flag_task=1;
                   break;
                  else if(replica_relability >= max_replica && flag_relability==0 &&  flag_task==0 && processor_available(i,j)==0)
